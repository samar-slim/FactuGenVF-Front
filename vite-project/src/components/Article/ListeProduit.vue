      ...
      <div class="bg-white p-2 rounded shadow">
        <div class="flex justify-between items-center mb-4">
          ...
          <button class="font-bold border bg-blue-800 text-white px-1 py-1 rounded-lg">
            <i class="fa-solid fa-plus" style="color: #ffffff;"></i> Nouveau Produit
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white p-6 rounded">
      <div class="flex items-center">
        <div class="w-1/3 flex space-x-2">
          ...
          <input 
            type="date" 
            class="w-full bg-gray-50 p-2 border rounded" 
          />
        </div>
        <div class="w-2/3 pl-4">
          <div class="relative">
            <input 
              type="text" 
              class="w-full bg-gray-50 p-2 pl-10 border rounded" 
    ...
    <table class="w-full text-xs text-left rtl:text-right text-gray-500 dark:text-gray-400">
      <thead class="text-xs text-white uppercase bg-blue-800 dark:bg-gray-700 dark:text-gray-400">
        <tr>
          ...
          <th scope="col" class="px-2 py-2 w-16">Référence</th>
          <th scope="col" class="px-2 py-2 w-12">Prix Achat</th>
          <th scope="col" class="px-2 py-2 w-12">Prix Unit.</th>
          <th scope="col" class="px-2 py-2 w-8">TVA</th>
          <th scope="col" class="px-2 py-2 w-8">Type Unité</th>
          <th scope="col" class="px-2 py-2 w-12">Action</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="produit in paginatedProduits" :key="produit._id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
          ...
          <td class="px-6 py-4">
            <a href="#" @click.prevent="editProduit(produit._id)" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  methods: {
    ...
    produitLoad() {
    axios.get('http://localhost:8080/api/produits/type/produit')
      .then(({ data }) => {
        this.produits = data;
      })
      .catch(error => {
