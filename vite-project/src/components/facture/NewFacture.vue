...
<div v-if="showErrorMessage" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mt-4" role="alert">
  <strong>Attention !</strong> Veuillez remplir tous les champs obligatoires du facture avant de finaliser.
</div>
<div v-if="modalOpenFinaliser" class="fixed inset-0 flex items-center justify-center bg-opacity-50 bg-gray-800 backdrop-blur-xs">
  <div class="max-w-md bg-white p-8 rounded shadow-lg">
    ...
    <div class="grid grid-cols-2 gap-4 mt-4">
      ...
      <div class="border p-4 flex items-center justify-center flex-col">
        <button  @click="saveDatafacture" class="font-semibold hover:text-blue-700">Enregistrer</button>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4 ">
  ...
  <div class="col-span-1">
      <div>
          ...
          <input type="text" v-model="facture.remarque" class="w-full form-input border border-gray-300 rounded-md px-1/2 py-2">
      </div>
      <div class="mt-4">
      <label class="block text-sm font-medium text-gray-700 mb-2">Condition de Règlement</label>
      <textarea class="w-full form-textarea border border-gray-300 rounded-md px-1/2 py-2" v-model="facture.condition" rows="3"></textarea>
    </div>
    <div class="mt-4">
      <label class="block text-sm font-medium text-gray-700 mb-2">Mode de Paiement</label>
      <select v-model="facture.paiement" class="w-full form-select border border-gray-300 rounded-md px-1/2 py-2">
...
<div class="flex items-center">
  <div class="relative">
    <input v-model="facture.remise" type="number" class="space-x-2 px-3 py-4 border border-gray-300 rounded-md bg-white text-sm" placeholder="Montant de la remise">
...
export default {
  ...
  this.facture.entrepriseId = this.user._id;
  this.facture.nomEntreprise = this.user.nomEntreprise;
  this.facture.telEntreprise = this.user.telEntreprise;
  this.facture.paysEntreprise = this.user.paysEntreprise;
  this.facture.adrEntreprise = this.user.adrEntreprise;
  this.facture.emailEntreprise = this.user.emailEntreprise;
  this.facture.siretEntreprise = this.user.siretEntreprise;
  this.facture.tvaEntreprise = this.user.tvaEntreprise;
},

validateForm() {
  const isEntrepriseSelected = this.facture.entrepriseId !== null && this.facture.entrepriseId !== '';
...
exportToPDF(factureId) {
 ...
 console.log('cliennttt selectioneee 88888888888888888',selectedClientId)
  ...
  selectOption(option) {
    ...
    this.remiseValue = this.facture.remise
...
return totalTTC;
  ...
  resetInputs() {
    ...
    // Logique pour finaliser le facture
...
async saveDatafacture() {
  ...
  if (!this.validateForm()) {
    toast.error('Veuillez remplir tous les champs obligatoires avant de soumettre le facture.');
    return;
  }
this.facture.clientId = this.selectedClientId;

this.facture.produitId = this.selectedArticleId;
this.facture.totalHT = this.totalHT;
    this.facture.totalTTC = this.totalTTC,
this.facture.remarque = this.remarque;
this.facture.userId= this.user._id;
...
try {
  ...
  if (responsefacture.status === 200 || responsefacture.status === 201) {
    ...
    this.factureLoad();
    this.facture = {};
   this.$router.push('user/ListeFacture' );
  } else {
    console.error('Erreur lors de l\'enregistrement du facture:', responsefacture.statusText);
    toast.error("Erreur lors de l'enregistrement du facture, veuillez réessayer.");
  }
} catch (error) {
