...
<br/>
    ...
    <div class="px-80 py-1  ml-40 justify-end  hover:text-white">
        <img  v-if="showImage"  src="c:\Users\hp\Downloads\Edit File Cute Color icon in PNG, SVG.jpeg" class="w-[180px]  justify-end">
    </div>
</td> 
        </tr>
            ...
            <table class="w-full text-left table-auto print:text-sm" id="table-items">
                ...
                <thead>
                    <tr class="text-white bg-blue-900 print:bg-gray-300 print:text-black">
                        ...
                        <th class="px-4 py-2 text-right border text-right text-black border  bg-white tabular-nums slashed-zero">{{ devis.totalHT}}$</th>
                    </tr>
    ...
    <div class="relative">
      ...
      <div v-show="dropdownOpen" class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-20">
        ...
        <button @click="selectOption('%')" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 w-full text-left"> (%)</button>
                  ...
                  <td class="px-4 py-2 text-right text-black border  bg-white tabular-nums slashed-zero" > {{ devis.totalTTC}} $</td>
                </tr>
...
<template>
    <div>
      <div class="bg-gray-100 py-2 p-1 mb-8 rounded shadow">
      <div class="bg-white py-2 rounded shadow">
        <div class="flex justify-between items-center mb-4">
          ...
          <div class="flex justify-end">
      <button @click="redirigerVersDevis" class="bg-blue-800 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded"><i class="pi pi-plus"></i> Ajouter Devis</button>
    </div>
        </div>
      </div>
    </div>
  ...
  <input type="text" id="table-search-users" class="block p-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Recherche avec Nom ,Ville ..." @click="handleSearchClick">
     ...
     <div v-if="selectedDevis.length > 0" class="flex space-x-2 justify-between mb-1">
      <div>
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
      ...
      <tbody>
        <tr v-for="devis in paginatedDevis" :key="devis._id" class="odd:bg-white cursor-pointer odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
          <td class="w-4 p-6">
            <div class="flex items-center">
    ...
    <nav class="flex items-center justify-between pt-4" aria-label="Table navigation">
      <span class="text-sm font-normal text-gray-500 dark:text-gray-400">Showing <span class="font-semibold text-gray-900 dark:text-white">{{ startItem }}</span> to <span class="font-semibold text-gray-900 dark:text-white">{{ endItem }}</span> of <span class="font-semibold text-gray-900 dark:text-white">{{ totalItems }}</span></span>
      <ul class="inline-flex items-center -space-x-px">
        <li>
          <button @click="prevPage" :disabled="currentPage === 1" class="px-3 py-2 ml-0 leading-tight text-gray-500 bg-white border border-gray-300 rounded-l-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"><i class="pi pi-chevron-left"></i></button>
        </li>
        <li v-for="page in totalPages" :key="page">
          <button @click="goToPage(page)" :class="{'text-blue-600 bg-blue-50 border-blue-300': currentPage === page, 'text-gray-500 bg-white border-gray-300 hover:bg-gray-100 hover:text-gray-700': currentPage !== page}" class="px-3 py-2 leading-tight">{{ page }}</button>
        </li>
        <li>
          <button @click="nextPage" :disabled="currentPage === totalPages" class="px-3 py-2 leading-tight text-gray-500 bg-white border border-gray-300 rounded-r-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"> <i class="pi pi-chevron-right"></i></button>
        </li>
      </ul>
    </nav>
  ...
  export default {
    ...
    paginatedDevis() {
      ...
      return this.devis.slice(start, end);
    },
    totalItems() {
    ...
    sendSelected() {
      console.log('Envoyer les devis sélectionnés:', this.selectedDevis);
    },
    editSelected() {
      console.log('Éditer les devis sélectionnés:', this.selectedDevis);
    },
    deleteSelected() {
      const deleteRequests = this.selectedDevis.map(id =>
    ...
    toggleSelectAll(event) {
      if (event.target.checked) {
        this.selectedDevis = this.devis.map(devis => devis._id);
      } else {
        this.selectedDevis = [];
      }
    },
      toggleMenu(id) {
            // Si showMenu est déjà défini sur id, le réinitialiser à null pour le masquer
            this.showMenu = this.showMenu === id ? null : id;
        },
        option1(id) {
            // Logique pour l'option 1
            console.log("Option 1 pour l'élément avec l'ID:", id);
        },
        option2(id) {
            // Logique pour l'option 2
            console.log("Option 2 pour l'élément avec l'ID:", id);
        },
        option3(id) {
            // Logique pour l'option 2
            console.log("Option 2 pour l'élément avec l'ID:", id);
    ...
    redirigerVersDevis() {
      // Utilisation de router.push pour naviguer vers la route du composant Devis
...
devisLoad() {
  ...
  <template>
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
        <h2 class="text-2xl mb-4">Éditer Devis</h2>
        <div id="pdf">
          <p>ID du devis : {{ formDevis?.id }}</p>
          <p>Client : {{ clientInfo?.nom }}</p>
          <p>Email : {{ clientInfo?.email }}</p>
          <div v-for="produit in formProduit" :key="produit._id">
            <p>Produit : {{ produit.nom_article }}</p>
            <p>Référence : {{ produit.reference }}</p>
            <p>Prix unitaire : {{ produit.prix_unitaire }}</p>
            <p>Quantité : {{ produit.quantity }}</p>
          </div>
        </div>
        <button @click="exportToPDF" class="mt-4 px-4 py-2 bg-blue-600 text-white rounded">Exporter en PDF</button>
        <button @click="close" class="mt-4 px-4 py-2 bg-red-600 text-white rounded">Fermer</button>
      </div>
    </div>
  </template>
    ...
    setup(props, { emit }) {
      ...
      const getDevisById = async (id) => {
        try {
          ...
          formProduit.value.forEach(produit => {
  ...
  <style scoped>
  /* Ajoutez ici vos styles pour le composant ShowDevis */
  </style>
  /********* showDevis l9dima */
  <template>
    <div>
      ...
      <div class="container mx-auto py-8 px-4" >
        <h1 class="text-2xl font-semibold mb-8" > Devis  {{ formDevis?.id }}</h1>
        <div class="flex justify-end">
...
<button type="button" @click="GenererFacture" class="px-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-s-lg hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:text-white dark:hover:bg-gray-700 dark:focus:ring-blue-500 dark:focus:text-white">
 Generer Facture
  </button>
...
setup(props) {
  ...
  const getDevisById = async (id) => {
    try {
      ...
      console.log('nlawej 3la id mta3 client', response.data.devis.clientId);
      formProduit.value = response.data.produitsSelectionnes;
      ...
      await getClientInfo(formDevis.value.clientId);
      await getProduitInfo(formDevis.value.produitId);
    } catch (error) {
      console.error("Erreur lors de la récupération du devis:", error);
    }
  };
...
<table class="border  w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
  <thead class="text-xs text-gray-700 uppercase bg-blue-800 dark:bg-gray-700 dark:text-gray-400">
      <tr>
