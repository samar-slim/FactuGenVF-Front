...
<div v-if="modalOpenFinaliser" class="fixed inset-0  items-center justify-center bg-opacity-50 bg-gray-800 backdrop-blur-xs">
  <div >
  <div class="-black h-screen p-60 ">
    <div class="row-span-2 grid h-2/3   bg-white  shadow-lg">
      <div class="flex items-center  justify-between p-2 md:p-9 border-b rounded-t ">
            <h3 class="text-xl  font-semibold text-gray-900 ...">Le devis est bien finalisé</h3>
            <button @click=" closeModalFinaliser" type="button" ><i class="fa-solid fa-xmark"></i></button>
          </div>
      <div class="row-span-2 border pl-6 pt-6 ">
        <h3 class="font-semibold">Envoyer par mail <i class="pi pi-send"></i></h3>

      </div>
      <div class="row-span-2 border pl-4 pt-4 ">
        <h3 class="font-semibold">Partager le lien:</h3>
      </div>
      <div class="row-span-2 border pl-4 pt-4 ">
        <button type="button" @click="exportToPDF" >Télécharger PDF:</button>
      </div>
      </div>
      </div>
      </div>
...
<button  @click="openModal1" class="bg-blue-800 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded"><i class="pi pi-plus"></i></button>
   ...
   <div >
      <div v-if="modalOpen1" class="fixed inset-0 flex items-center w-[1500px] justify-center bg-opacity-50 bg-gray-800 backdrop-blur-xs">
        <!-- Contenu de la modal -->
        <div class="relative bg-white rounded-lg shadow ...">
          <!-- En-tête de la modal -->
          <div class="flex items-center justify-between p-4 md:p-9 border-b rounded-t ">
            <h3 class="text-xl font-semibold text-gray-900 ...">Nouveau Client</h3>
...
<<<<<<< HEAD
  ...
  <button @click="()=> singleSelect.quantity ++ " class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-2 rounded-l">+</button> 
  <span class="bg-white-500  text-black font-bold py-2 px-4 border ">{{ singleSelect.quantity }}</span>
  <button @click="()=> singleSelect.quantity -- " class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-2 rounded-r">-</button>
</td>
      ...
      <td class="px-5 py-2 border tabular-nums slashed-zero">{{ singleSelect.quantity  * singleSelect.prix}}</td>
...
=======
              ...
              <tbody>
                ...
                <tr>
                  <button id="add-row-button" class="px-6 py-2 border text-black-900 bg-gray-100"><span class="pi pi-plus"></span>Ajouter ligne</button>
>>>>>>> f53c9e6ac6a7bbc9c72eeaa0ac161d0988572462
...
=======
    <!-- Section gauche avec les champs Remarque, Condition de Règlement et Mode de Paiement -->
>>>>>>> f53c9e6ac6a7bbc9c72eeaa0ac161d0988572462
    <div class="col-span-1">
        ...
        <div class="mt-4">
            ...
            <select class="w-full form-select border border-gray-300 rounded-md px-1/2 py-2">
<<<<<<< HEAD
...
=======
    ...
    <!-- Section droite avec le tableau -->
    <div class="col-span-1 p-14" v-if="!isOpen && !modalOpen1" >
        <div class="relative  shadow-md sm:rounded-lg">
            <table class="w-full text-left table-auto print:text-sm" id="table-items">
                <!-- En-têtes du tableau -->
>>>>>>> f53c9e6ac6a7bbc9c72eeaa0ac161d0988572462
...
<<<<<<< HEAD
                        <th class="px-4 py-2 text-right border text-right text-black border  bg-white tabular-nums slashed-zero">{{ calculateTotalHT() }}$</th>
                    </tr>
                    
                   
=======
                        <th class="px-4 py-2 text-right border text-right text-black border  bg-white tabular-nums slashed-zero">0.00 £</th>
                    </tr>
                    <tr class="text-white bg-blue-900 print:bg-gray-300 print:text-black" >
...
<td class="px-4 py-2 text-right border text-center text-black border bg-white tabular-nums slashed-zero">
  <div class="flex items-center">
    <div class="relative">
      <input v-model="remiseValue" type="number" class="space-x-2 px-3 py-4 border border-gray-300 rounded-md bg-white text-sm" placeholder="Montant de la remise">
      ...
      <div v-show="dropdownOpen" class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-20">
        ...
        <button @click="selectOption('%')" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 w-full text-left"> (%)</button>
                  ...
                  <td class="px-4 py-2 text-right text-black border  bg-white tabular-nums slashed-zero"> {{ calculerTotalTTC() }} $</td>
=======
                    <!-- Ligne du tableau -->
                    <tr class="text-white bg-blue-900 print:bg-gray-300 print:text-black">
                        <td class="invisible"></td>
                        <td class="invisible"></td>
                        <td class="px-4 py-2 text-right border"><span class="flag-icon flag-icon-hu print:hidden"></span>Remise par ligne</td>
                        <td class="px-4 py-2 text-right border text-right text-black border  bg-white tabular-nums slashed-zero">0.00 £</td>
                    </tr>  <tr class="text-white bg-blue-900 print:bg-gray-300 print:text-black" >
                  <td class="invisible"></td>
                  <td class="invisible"></td>
                  <td class="px-4 py-2 font-extrabold text-right border"> Total TTC </td>
                  <td class="px-4 py-2 text-right text-black border  bg-white tabular-nums slashed-zero"> 0.00 £</td>
>>>>>>> f53c9e6ac6a7bbc9c72eeaa0ac161d0988572462
                </tr>
...
<<<<<<< HEAD
  ...
  if (this.selectedOption === '$') {
    return totalHT - this.remiseValue; 
         ...
         this.produit.forEach((item)=>{
          if(item._id === event.target.value){
           const isExist =     this.tableOfSelectedOptions.some(item=> { 
              ...
              this.$emit('ProduitSelected', selectedArticleId);
          }
...
saveDataDevis() {
  ...
  console.log("ID du client sélectionné :", this.selectedClientId);
...
showCustomizationPopup: false,  
  ...
  console.log('Close event received');
    ...
    handleSubmit() {
      ...
      // Réinitialisez les données de l'article
      this.article = {
        title: ''
        // Réinitialisez d'autres propriétés de l'article ici
      };
      // Fermez le modal
      this.showModal = false;
    },
