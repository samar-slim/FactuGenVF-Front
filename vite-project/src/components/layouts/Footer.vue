            ...
            <form @submit.prevent="handleSubmit()">
                <div class="relative flex flex-col items-center lg:flex-row justify-center lg:space-x-3">
                    <div class="relative w-full lg:mb-0 font-theme-content">
                        <input v-model="email" @input="emailError = false" class="w-full py-3 px-5 text-sm text-gray-600 border-0 shadow-md rounded-full focus:ring-4 focus:ring-blue-300" type="text" name="newsletter_email" placeholder="Enter your email address" />
                        <div v-show="emailError">
                            <div class="absolute w-full text-xs italic px-2 py-1 bg-theme-secondary text-white rounded-full">Whosps, make sure it's an email</div>
                        </div>
                    </div>
